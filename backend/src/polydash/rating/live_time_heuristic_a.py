...
from polydash.model.plagued_node import TransactionFetched, PlaguedBlock, PlaguedTransactionFound, \
    PlaguedTransactionMissing, PlaguedNode
...
def process_block(block_number, block_hash, base_fee, block_timestamp, transactions, author_node, plagued_node):
    ...
    for tx_hash, tx_data in transactions.items():
        if tx_hash in pending_txs:
            ...
            plagued_block.tx_found.add(
                PlaguedTransactionFound(tx_hash=tx_hash, signer=tx_data["from"], nonce=tx_data["nonce"],
                                        miner_fee_block=str(miner_fee_block), miner_fee_pool=str(miner_fee_pool),
                                        block=block_number))
        else:
            transactions_missing.append(tx_hash)
            plagued_block.tx_missing.add(
                PlaguedTransactionMissing(tx_hash=tx_hash, signer=tx_data["from"], nonce=tx_data["nonce"],
                                          miner_fee_block=str(get_effective_tip(base_fee, tx_data["gas_fee_cap"],
                                                                                tx_data["gas_tip_cap"])),
                                          block=block_number))
...
def pending_transactions_by_price_and_nonce(block_timestamp, base_fee):
    pending_txs = {}
    lower_bound = (block_timestamp * 1000) - 300000
    upper_bound = block_timestamp * 1000
...
def main_loop():
    while True:
        try:
            ...
            with orm.db_session:
                plagued_node = PlaguedNode.get_or_insert(pubkey=validated_by)
